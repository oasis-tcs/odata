...
WITH 
  -- traversal needs a stable sibling order; can be rowid if selecting from hierarchy table, or row_number() if preceding transformation
	...
	(
		SELECT *, CAST(RIGHT ('000' + CAST(ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS varchar(8000)), 4) AS varchar(8000)) 
...
--,
--  trafo_1_result
--(ID,SuperOrdinate,Name,SiblingIndex) AS
--( -- result of descendants(...), use row_id as sibling order
--    SELECT SalesOrganization.*, SalesOrganization.rowid
